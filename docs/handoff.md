...
安全加固发布时在本地和生产分别完成 86 项测试；生产外部验收确认 HTTPS 200、HTTP 301、Node 仅监听回环地址、公网 3000 拒绝连接、安全响应头生效、恶意 Origin 被拒绝，且线上 `dist` 与本地验证产物的全文件清单哈希一致。PDF 冷启动修复发布时，完整测试已增至 88 项。
