...
所有已登录用户在两种分析模式下均支持 MP3、M4A、WAV、OGG 本地录音。录音上限为 100 MiB、60 分钟，按 4 MiB 分块保存到私有目录，核验真实格式与时长后转换为 16 kHz 单声道 PCM WAV。元信息解析在限时、限内存 Worker 内运行，转换进程最多运行 120 秒。应用只分析用户检查并确认后的文字，不做声学或情绪评分。首版不展示飞书妙记，不开放授权、回调或链接导入接口。
