...
新增录音分块上传、任务查询和确认接口；两种分析请求均可传 materialId，由服务端校验所有者和确认稿。上传连续 30 分钟没有成功分块会自动结束并清理；主动取消、上传超时和已清理任务必须重新创建，不能调用重试接口恢复。首版不提供飞书妙记 OAuth 或链接导入。完整契约见 [录音接口](audio-minutes-operations.md#api-概览)。
...
当前分析接口是同步长请求，调用方应允许最多 11 分钟，并在等待期间保持连接。服务端 AI SDK 在 10 分钟主动超时，生产 Nginx 在 11 分钟结束代理等待。常见稳定错误码：

| HTTP | `code` | 含义 |
|---|---|---|
| `504` | `AI_UPSTREAM_TIMEOUT` | 模型在应用超时内没有完成 |
| `503` | `AI_PROVIDER_BUSY` | 模型服务繁忙 |
| `429` | `AI_PROVIDER_RATE_LIMITED` | 模型侧限流 |
| `429` | `ANALYSIS_IN_PROGRESS` | 同一用户已有分析在途 |
| `502` | `AI_PROVIDER_ERROR` | 其他模型调用失败 |

调用方取消或断开连接时，服务端会取消仍在途的模型请求，不返回可消费的 HTTP 响应。失败和取消都不会创建报告。
